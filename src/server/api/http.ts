...
export function failure(error:unknown){if(error instanceof ApiError)return json({error:error.message,code:error.code},error.status);if(error instanceof ZodError)return json({error:"Invalid request",code:"VALIDATION_ERROR",issues:error.issues},400);console.error(error);return json({error:`Internal error: ${error instanceof Error ? error.message : String(error)}`,code:"INTERNAL_ERROR"},500);}
